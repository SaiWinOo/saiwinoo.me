    ...
    <ol class="group/list">
      {
        experiences.map(experience => (
          <li key={experience.link} class="mb-12">
            <div class="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
              ...
              <header class="z-10 mb-2 mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500 sm:col-span-2" aria-label="2018 to Present">{experience.year}
              </header>
              <div class="z-10 sm:col-span-6">
                <h3 class="font-medium leading-snug text-slate-200">
                  <div>
                    <a class="inline-flex items-baseline font-medium leading-tight text-slate-200 hover:text-teal-300 focus-visible:text-teal-300  group/link text-base" href={experience.link} target="_blank" rel="noreferrer noopener" aria-label="Lead Engineer at Upstatement (opens in a new tab)">
                      <span class="absolute -inset-x-4 -inset-y-2.5 hidden rounded md:-inset-x-6 md:-inset-y-4 lg:block"></span>
                      <span>{experience.position} · <span class="inline-block">
                        {experience.company}
                ...
                <p class="mt-2 text-sm leading-normal text-gray-400">
                  {experience.summary}
                </p>
                <ul class="mt-2 flex flex-wrap" aria-label="Technologies used">
                  {
                    experience.technologies.map(technology => (
                      <li key={technology} class="mr-1.5 mt-2">
